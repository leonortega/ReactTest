...
- Configure nginx in the frontend container to proxy `/api` to `127.0.0.1:8080` and set `VITE_API_BASE_URL` to the value from an `.env` variable at build time in GitHub Actions. Edits should only be made to the existing `frontend/nginx.conf` in the frontend folder; do not create `nginx.conf` elsewhere.
